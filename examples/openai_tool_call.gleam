...
fn handle_round(
  ...
  starlet.Chat(starlet.ToolsOn, starlet.FreeText, starlet.Ready, openai.Ext),
  ...
  case starlet.has_tool_calls(turn) {
    ...
    True -> {
      ...
      io.println("GPT: " <> starlet.text(final_turn))
...
fn send_chat(
  ...
  starlet.Turn(starlet.ToolsOn, starlet.FreeText, openai.Ext),
  starlet.StarletError,
) {
