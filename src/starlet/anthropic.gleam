...
/// Clears `thinking` since the synthesized turn carries no thinking content.
pub fn assistant_with_tool_calls(
  ...
  starlet.Chat(
    ...
    ext: Ext(..chat.ext, thinking: option.None),
  )
}

/// Replace the message history and transition the chat to `Ready`.
///
/// Use this to rehydrate an Anthropic chat from a stored transcript before
...
/// `Error(InvalidArgument)` if `messages` is empty. Clears `thinking` since
/// the rehydrated transcript has no associated last-response metadata.
pub fn from_messages(
  ...
  Ok(
    starlet.Chat(..chat, messages:, ext: Ext(..chat.ext, thinking: option.None)),
  )
}
